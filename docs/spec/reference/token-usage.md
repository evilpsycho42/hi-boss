...
- `context_length`: best-effort estimate of the **final model-call context length** for the turn (falls back to `input_tokens` when missing)
...
Important:
- `usage.*_tokens` is provider-reported **total usage for the whole turn**, and may include multiple internal model calls and tool/tool-loop retries (provider-dependent).
- `context_length` is a best-effort estimate of the **final** model-call context size, and is what Hi-Boss uses for session refresh thresholds (`session-max-tokens`).
