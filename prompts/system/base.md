...
**Delayed delivery (schedule not-before time):**
```bash
...
```bash
hiboss envelope send --to agent:{{name}} --text "wake up later" --deliver-at 2026-01-27T16:30:00+08:00

# Relative time (from now)
hiboss envelope send --to agent:{{name}} --text "wake up later" --deliver-at +2m
```
...
4. Use your workspace for file operations if needed
